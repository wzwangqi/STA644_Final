...
For this project, we utilized methods learnt from this class using both time series and spatial models and applied them on the terrorist attack dataset from Global Terrorism Database. We used ARIMA models to forecast attack counts for different regions of the world and were able to obtain models that match the current global political situations. In the spatial context, both frequentist and bayesian version of Car and Mar model can be able to capture the spatial patterns of terroristic attacks. In addition, spline model has been used to predict the successful rate if a terrorstic attack took place based on the observed data. In future, we could try bayesian version spline model to do the predication. Further compared the results with the regular spline model since the regular spline model is not good at uncertainty prediction.
